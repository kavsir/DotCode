...
class Coder:
    ...
    def run_one(self, user_message, preproc):
        ...
        if intent == "question":
            ...
            self._handle_question(message, is_architecture=False)
        ...
        if intent == "architecture":
            self.pending_question = True
            self._auto_add_context(message, max_files=3)
            self._handle_question(message, is_architecture=True)
            self.pending_question = False
            return
    ...
    def _handle_question(self, message, is_architecture=False):
        ...
        if is_architecture and self.code_graph:
        ...
        if not self.abs_fnames and not self.abs_read_only_fnames:
            self.io.tool_output(
                "💡 Tip: Use /add <file> to add files to the chat so I can answer questions about"
                " your code."
            )
...
Chỉ trả về JSON hợp lệ."""
    ...
    def send_message(self, inp):
        ...
        if hasattr(self, "model_router"):
            ...
            if selected_model_name != self.main_model.name:
                self.io.tool_output(
                    f"🧠 Switching to {selected_model_name} for {intent} task"
        ...
        try:
            ...
            for edit in edits:
                ...
                if risk == RiskLevel.LOW:
                    ...
                    self.io.tool_output(
                        f"🚨 Important change in {fname} - please review carefully:"
                    )
                    apply_change = self.io.confirm_ask("Apply this change?")
                ...
                if apply_change:
                    ...
                    if (
                        hasattr(self, "code_graph")
                        and self.code_graph
                        and hasattr(self.code_graph, "sage")
                    ):
                        if search.strip():
                            sym_id = f"{fname}::{search.splitlines()[0][:30]}"
                        else:
                            sym_id = f"{fname}::new_file"
    ...
    def _handle_command(self, message):
        ...
        editable_list = (
            ", ".join([os.path.relpath(f, self.root) for f in self.abs_fnames])
            if self.abs_fnames
            else "NONE"
        )
    ...
    **🔓 DotCode Special Permission:**
    ...
    def _handle_contextual_yes(self, state):
        ...
        if state.question_text:
            ...
            # Gửi lại prompt gốc để AI thực hiện
            self._handle_command(state.question_text)