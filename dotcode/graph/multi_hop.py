...
class MultiHopEngine:
    ...
    def get_k_hop_neighbors(self, symbol_id: str, k: int = 2, 
    ...
    def find_shortest_path(self, source_id: str, target_id: str, 
                          max_depth: int = 5,
                          edge_types: List[str] = None) -> Optional[List[Dict]]:
        """
    ...
    def find_community_bridges(self, community_id_1: int, community_id_2: int,
    ...
    def multi_hop_query(self, query_text: str, 
                       graphrag_communities: Dict = None,
                       max_depth: int = 3) -> str:
        """
        ...
        match = re.search(r"(?:calls?|gọi)\s+(?:functions?|hàm)\s+(?:called by|được gọi bởi)\s+(\w+)", query_text, re.IGNORECASE)
        if match:
            ...
            if syms:
                ...
                for callee in callees:
                    ...
                    for caller in callers:
                        results.append(f"- {caller['name']} (calls {callee['name']} which is called by {target_name})")
                if results:
                    return "Multi-hop results:\n" + "\n".join(results[:10])
        
        # Pattern: "is X connected to Y" -> path finding
        match = re.search(r"(?:is|are|có phải)\s+(\w+)\s+(?:connected to|related to|liên quan đến)\s+(\w+)", query_text, re.IGNORECASE)
        if match:
            ...
            if source_syms and target_syms:
                path = self.find_shortest_path(source_syms[0]["id"], target_syms[0]["id"], max_depth)
                if path:
                    steps = " → ".join([f"{s['symbol'].name}({s.get('edge_type', '')})" for s in path])
                    return f"Path from {source_name} to {target_name}: {steps}"
                else:
                    return f"No path found between {source_name} and {target_name} within {max_depth} steps."
